...
class Concorrente2: UIViewController{
    ...
    @IBAction func votarButtonClicked(_ sender: Any){
        ...
        if (vezes == 0){
            ...
            databaseReference.child("ios").child("concursos").child("reis").child("votos").child("2Tatiana").childByAutoId().setValue(ratingStackView.starsRating)
            vezes += 1;
        }
        else{
