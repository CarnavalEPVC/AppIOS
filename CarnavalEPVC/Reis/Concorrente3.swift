...
class Concorrente3: UIViewController{
    var vezes: Int=0;
    ...
    @IBAction func votarButtonClicked(_ sender: Any){
        ...
        if (vezes == 0){
            ...
            databaseReference.child("ios").child("concursos").child("reis").child("votos").child("3InesNovo").childByAutoId().setValue(ratingStackView.starsRating)
            vezes += 1;
        }
        else{
