...
class con6: UIViewController{
    ...
    @IBAction func votarButtonClicked(_ sender: Any){
        ...
        if (vezes == 0){
            ...
            databaseReference.child("ios").child("concursos").child("gottalent").child("votos").child("6").childByAutoId().setValue(ratingStackView.starsRating)
            vezes += 1;
        }
        else{
